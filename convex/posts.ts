  ...
  handler: async (ctx) => {
    ...
    return await Promise.all(
      posts.map(async (post)=>{
        const resolveImgaeUrl=post.imageStorageId!==undefined?
          await ctx.storage.getUrl(post.imageStorageId):null;

          return{
            ...post,
            imageUrl:resolveImgaeUrl,
          }
      })
    )
...
export const generateImageUploadOrl=mutation({
...
export const searchPosts=query({
  ...
  handler:async(ctx,args)=>{
    ...
    const titleMatches=await ctx.db
        ...
        (q)=>q.search("title",args.term)).take(limit);
    ...
    if(results.length<limit){
      const bodyMatches=await ctx.db
        ...
        (q)=>q.search("body",args.term)).take(limit - results.length);
